...
class RulerView(context: Context?, attrs: AttributeSet?, defStyleAttr: Int) :
	...
	fun initViewParam(
		...
		mMaxValue = maxValue
		mMinValue = minValue
		mPerSpanValue = (perSpanValue * mMinSpanCount).toInt().toFloat()
		mTotalLine = ((mMaxValue * mMinSpanCount - mMinValue * mMinSpanCount).toInt() / mPerSpanValue + 1).toInt()
	...
	fun mResultTextColor(mResultTextColor: Int) {
	...
	private fun drawIndicator(canvas: Canvas) {
		...
		if (mIndicatorType == LINE) {
			canvas.drawLine(
				srcPointX.toFloat(), resultHeight.toFloat(), srcPointX.toFloat(), (mIndcatorHeight + resultHeight).toFloat(),
				mIndicatorPaint!!
			)
